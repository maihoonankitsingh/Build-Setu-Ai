...
export async function POST(request: NextRequest) {
  try {
    ...
    await setUserPlan(order.userId, order.planId || "pro", order.planName || "BuildSetu Pro");
    ...
    await appendHistory({
      ...
      description: `${order.planName || "Plan"} upgrade +${order.credits.toLocaleString("en-IN")} credits`,
      razorpayOrderId: order.razorpayOrderId,
      razorpayPaymentId,
    });
    ...
    console.error("plans/verify failed", error);
    return NextResponse.json({ ok: false, error: "PLAN_VERIFY_FAILED" }, { status: 500 });
  }
}
